...
function renderAdminCategories() {
    adminCategoriesList.innerHTML = categories.map(category => `
        <tr ${category.protected ? 'style="opacity:0.85"' : ''}>
            <td>
                ${category.id}
                ${category.protected ? '<span style="font-size:0.7rem;color:#888;margin-left:4px">(fixo)</span>' : ''}
            </td>
            <td>${category.label}</td>
            <td>
                <div class="admin-table-actions">
                    <button class="btn-icon" onclick="editCategory('${category.id}')" title="Editar nome">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                            <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path>
                            <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path>
                        </svg>
...
window.deleteProduct = async function(id) {
    ...
    try {
        // Antes isso so removia o produto da lista em memoria (sem chamar a API),
        // por isso ele "voltava" ao recarregar a pagina: nada era excluido no servidor.
        await apiAdminDel(`/api/admin/products/${id}`);
        await loadAdminData();
        renderAdminProducts();
        updateStats();
        ...
        if (typeof document !== 'undefined') {
            ...
            try {
                const contactAfterSave = {
                    ...
                    instagram: String(document.getElementById('edit-instagram')?.value || '').trim(),
                };
