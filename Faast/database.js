...
async function saveProducts(products) {
  ...
  const keepIds = prods.map(p => Number(p.id)).filter(n => Number.isFinite(n));

  await pool.query('BEGIN');
  try {
    // Remove do banco os produtos que NAO estao mais na lista enviada (ex.: produto
    // excluido no admin). Sem isso, o save so fazia INSERT/UPDATE (upsert) e o produto
    // "deletado" continuava no Postgres, voltando a aparecer apos recarregar a pagina.
    // As imagens associadas sao removidas automaticamente via ON DELETE CASCADE.
    await pool.query('DELETE FROM laet_products WHERE NOT (id = ANY($1::bigint[]))', [keepIds]);
...
async function saveCategories(categories) {
  ...
  const keepIds = cats.map(c => String(c.id || '')).filter(Boolean);

  await pool.query('BEGIN');
  try {
    // Remove do banco as categorias que NAO estao mais na lista enviada (ex.: categoria
    // excluida no admin). Mesmo bug do saveProducts: antes so havia upsert, nunca delete.
    // Categorias marcadas como "protected" nunca sao removidas aqui (camada extra de
    // seguranca, alem da checagem que ja existe nas rotas do server.js).
    await pool.query(
      'DELETE FROM laet_categories WHERE NOT (id = ANY($1::text[])) AND protected = FALSE',
      [keepIds]
    );
