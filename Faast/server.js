...
app.delete('/api/admin/categories/:id', requireAdmin, async (req, res, next) => {
  try {
    ...
    // Impede excluir uma categoria que ainda tem produtos vinculados (o banco tem
    // ON DELETE RESTRICT nessa FK, entao isso geraria um erro 500 sem essa checagem).
    const products = await database.getProducts();
    const inUse = products.some(p => String(p.category) === catId);
    if (inUse) {
      return res.status(400).json({ error: 'Categoria possui produtos vinculados. Mova ou exclua os produtos antes de remover a categoria.' });
    }
